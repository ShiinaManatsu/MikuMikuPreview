    ...
    public class MaterialDropdown : UIBehaviour, IOptionDataListContainer
    {
        ...
        private Vector3 CalculatedPosition()
        {
            ...
            if (m_VerticalPivotType == VerticalPivotType.BelowBase || m_VerticalPivotType == VerticalPivotType.AboveBase)
            {
                ...
                position.y -= m_ExpandedSize.y * scaler.scaleFactor / 2 * flipper;
                position.y -= baseHeight * scaler.scaleFactor / 2 * flipper;
            }
            else if (m_VerticalPivotType == VerticalPivotType.Top || m_VerticalPivotType == VerticalPivotType.Bottom)
            {
                position.y -= m_ExpandedSize.y * scaler.scaleFactor / 2 * flipper;
                position.y += itemHeight * scaler.scaleFactor / 2 * flipper;
            ...
            {
                position.x -= m_BaseTransform.GetProperSize().x * scaler.scaleFactor / 2 * flipper;
                position.x += m_ExpandedSize.x * scaler.scaleFactor / 2 * flipper;
            }

            RectTransform rootCanvasRectTransform = MaterialUIScaler.GetParentScaler(m_DropdownPanel).GetComponent<RectTransform>();

            //  Left edge
            float canvasEdge = rootCanvasRectTransform.position.x / scaler.scaleFactor - rootCanvasRectTransform.rect.width / 2;
            float dropdownEdge = position.x / scaler.scaleFactor - m_ExpandedSize.x / 2;
            if (dropdownEdge < canvasEdge + m_MinDistanceFromEdge)
            {
                position.x += (canvasEdge + m_MinDistanceFromEdge - dropdownEdge) * scaler.scaleFactor;
            }

            //  Right edge
            canvasEdge = rootCanvasRectTransform.position.x / scaler.scaleFactor + rootCanvasRectTransform.rect.width / 2;
            dropdownEdge = position.x / scaler.scaleFactor + m_ExpandedSize.x / 2;
            if (dropdownEdge > canvasEdge - m_MinDistanceFromEdge)
            {
                position.x -= (dropdownEdge - (canvasEdge - m_MinDistanceFromEdge)) * scaler.scaleFactor;
            }

            //  Top edge
            canvasEdge = rootCanvasRectTransform.position.y / scaler.scaleFactor + rootCanvasRectTransform.rect.height / 2;
            dropdownEdge = position.y / scaler.scaleFactor + m_ExpandedSize.y / 2;
            if (dropdownEdge > canvasEdge - m_MinDistanceFromEdge)
            {
                position.y -= (dropdownEdge - (canvasEdge - m_MinDistanceFromEdge)) * scaler.scaleFactor;
            }
            ...
            if (dropdownEdge < canvasEdge + m_MinDistanceFromEdge)
            {
                position.y += ((canvasEdge + m_MinDistanceFromEdge) - dropdownEdge) * scaler.scaleFactor;
            }
        ...
        private float CalculateMaxItemWidth()
        {
            ...
            for (int i = 0; i < m_OptionDataList.options.Count; i++)
            {
                ...
                {
                    currentWidth = textGenerator.GetPreferredWidth(m_OptionDataList.options[i].text, textGenerationSettings) / scaler.scaleFactor;
                    currentWidth += 16;
                }
